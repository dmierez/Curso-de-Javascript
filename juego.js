...
class Juego {
    ...
    iluminarSecuencia() {
        for (let i = 0; i < this.nivel; i++) {
            const color = this.trasnformarAcolor(this.secuencia[i])
    ...
    ganoElJuego() {
        swal('Diego dice', 'Felicitaciones :) ganaste el juego!', 'success')
        .then(this.inicializar)
    }

    perdioElJuego() {
        swal('Diego dice', 'OPS! PERDISTE :(', 'error')
        .then(() => {
            this.eliminarEventosClick()
            this.inicializar()
        })
